    ...
    def __call__(self, data):
        for key in self.keys:
            if key in data:
                ...
                #data[key] = np.array(CenterCrop((448, 448))(data[key]), dtype=np.float32)
    ...
    def __call__(self, data):
        for key in self.keys:
            if key in data:
                ...
                data[key] = RandomAffine(0, shear=10, scale=(0.8,1.2))(data[key])
...
class ImgNormalize():
    ...
    def __call__(self, data):
        for key in self.keys:
            if key in data:
                data[key] = Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))(data[key])
