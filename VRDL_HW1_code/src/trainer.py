...
class Trainer():
    ...
    def save_ckpt(self, ep, acc):
        save_name = f"ep={ep:0>3}-acc={acc:.4f}"
    ...
    def test_step(self):
        classes_list = read_classes(os.path.join(self.params.data_root, 'classes.txt'))
        self.model.eval()
        with open(os.path.join(self.params.data_root, 'answer.txt'), 'w') as fp:
    ...
    def ensemble(self, ModelList, dataset):
        ...
        with open(os.path.join(self.params.data_root, 'answer.txt'), 'w') as fp:
            with torch.no_grad():
                for batch_data in self.test_loader:
                    images = batch_data['image'].to(self.device)
                    preds = torch.sum(torch.stack([m(images) for m in self.ModelList]), dim=0).argmax(dim=1)
                    for Id, pred in zip(batch_data['id'], preds):
                        fp.write(f"{Id} {classes_list[pred]}\n")
                        print(f"{Id} {classes_list[pred]}")
